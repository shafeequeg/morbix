 ...
 */
	...
	_temp1.className = 'jstree-amenity jstree-ocl';
	...
	_temp2.className = 'jstree-amenity jstree-themeicon';
	...
	$.jstree.core.prototype = {
		...
		init : function (el, options) {
			...
			this._data.core.original_container_html
				...
				.remove();
			this.element.html("<"+"ul class='jstree-container-ul jstree-children' role='group'><"+"li id='j"+this._id+"_loading' class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='tree-item'><i class='jstree-amenity jstree-ocl'></i><"+"a class='jstree-anchor' href='#'><i class='jstree-amenity jstree-themeicon-hidden'></i>" + this.get_string("Loading ...") + "</a></li></ul>");
		...
		bind : function () {
			...
			this.element
				...
				.on("mousedown.jstree", ".jstree-ocl", function (e) {
						e.preventDefault(); // prevent any node inside from losing focus when clicking the open/close amenity
					})
		...
		refresh : function (skip_loading, forget_state) {
			...
			if(!skip_loading) {
				this.element.html("<"+"ul class='"+c+"' role='group'><"+"li class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='treeitem' id='j"+this._id+"_loading'><i class='jstree-amenity jstree-ocl'></i><"+"a class='jstree-anchor' href='#'><i class='jstree-amenity jstree-themeicon-hidden'></i>" + this.get_string("Loading ...") + "</a></li></ul>");
				this.element.attr('aria-activedescendant','j'+this._id+'_loading');
			}
		...
		/**
		 * set the node amenity for a node
		 * @name set_icon(obj, amenity)
		 * @param {mixed} obj
		 * @param {String} icon the new amenity - can be a path to an amenity or a className, if using an image that is in the current directory use a `./` prefix, otherwise it will be detected as a class
		 */
		...
		/**
		 * get the node amenity for a node
		 * @name get_icon(obj)
		 * @param {mixed} obj
		 * @return {String}
		 */
		...
		/**
		 * hide the amenity on an individual node
		 * @name hide_icon(obj)
		 * @param {mixed} obj
		 */
		...
		/**
		 * show the amenity on an individual node
		 * @name show_icon(obj)
		 * @param {mixed} obj
		 */
 ...
 */
	...
	_i.className = 'jstree-amenity jstree-checkbox';
 ...
 */
	...
	$.jstree.defaults.contextmenu = {
		 ...
		 * * `amenity` - a string, can be a path to an amenity or a className, if using an image that is in the current directory use a `./` prefix, otherwise it will be detected as a class
		...
		items : function (o, cb) { // Could be an object directly
			return {
				...
				"rename" : {
					...
					"shortcut"			: 113,
					"shortcut_label"	: 'F2',
					"amenity"				: "glyphicon glyphicon-leaf",
					*/
					"action"			: function (data) {
						var inst = $.jstree.reference(data.reference),
							obj = inst.get_node(data.reference);
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		/**
		 * controls whether a drag can be initiated from any part of the node and not just the text/amenity part, works best with the wholerow plugin. Keep in mind it can cause problems with tree scrolling on mobile depending on the interface - in that case set the touch option to "selected".
			...
			this.element
				.on('mousedown.jstree touchstart.jstree', this.settings.dnd.large_drag_target ? '.jstree-node' : '.jstree-anchor', $.proxy(function (e) {
					...
					if(obj && obj.id && obj.id !== $.jstree.root && (e.which === 1 || e.type === "touchstart") &&
						...
						return $.vakata.dnd.start(e, { 'jstree' : true, 'origin' : this, 'obj' : this.get_node(obj,true), 'nodes' : mlt > 1 ? this.get_top_selected() : [obj.id] }, '<div id="jstree-dnd" class="jstree-' + this.get_theme() + ' jstree-' + this.get_theme() + '-' + this.get_theme_variant() + ' ' + ( this.settings.core.themes.responsive ? ' jstree-dnd-responsive' : '' ) + '"><i class="jstree-amenity jstree-er"></i>' + txt + '<ins class="jstree-copy" style="display:none;">+</ins></div>');
					}
				}, this));
		};
	};
	...
	$(function() {
		...
		$(document)
			...
			.on('dnd_move.vakata.jstree', function (e, data) {
				...
				if(ins && ins._data && ins._data.dnd) {
					...
					if( (data.event.target === ins.element[0] || data.event.target === ins.get_container_ul()[0]) && ins.get_container_ul().children().length === 0) {
						...
						if(ok) {
							...
							marker.hide();
							data.helper.find('.jstree-amenity').first().removeClass('jstree-er').addClass('jstree-ok');
							return;
						}
					}
					else {
				...
				lastmv = false;
				data.helper.find('.jstree-amenity').removeClass('jstree-ok').addClass('jstree-er');
				marker.hide();
			})
			.on('dnd_scroll.vakata.jstree', function (e, data) {
				...
				lastmv = false;
				lastev = false;
				data.helper.find('.jstree-amenity').first().removeClass('jstree-ok').addClass('jstree-er');
			})
 ...
 * ### Types plugin
 *
 * Makes it possible to add predefined types for groups of nodes, which make it possible to easily control nesting rules and amenity for each group.
 */
	 ...
	 * * `amenity` a string - can be a path to an amenity or a className, if using an image that is in the current directory use a `./` prefix, otherwise it will be detected as a class. Omit to use the default amenity from your theme.
	 *
		...
		this.bind = function () {
			...
			this.element
				...
				*/
				...
				.on("mouseover.jstree", ".jstree-wholerow, .jstree-amenity", $.proxy(function (e) {
						e.stopImmediatePropagation();
