...
//        File Directory
    ...
    public function view()
    {
        ...
        $this->page_data['result']= $this->Data->select_items('',$data)->result();
        $this->page_data['page_name'] = 'Item';
        $this->load->view('Index',$this->page_data);
    }
    public function create()
    {
        //$data1['batch']=$this->security->xss_clean($this->input->post('item'));
        $data1['item_name']=$this->security->xss_clean($this->input->post('item'));
        $q1 = $this->Data->select_items_checking('',$data1);
        if ($q1->num_rows() == 0) {
            ...
            redirect(base_url() . 'godown/item', 'refresh');
        }else{
            $data1['flashdata_msg'] = 'Sorry.. Item Alredy Exist. Please Try Again!';
            $data1['flashdata_type'] = 'error';
            $data1['flashdata_title'] = 'Error !!';
            $this->session->set_flashdata($data1);
            redirect(base_url() . 'godown/item', 'refresh');
