                ...
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body">
                        <table class="table table-striped table-bordered table-hover table-header-fixed" id="sample_1">
                            ...
                            <?php
                            foreach ($result as $r):
                                // $customer1 = $r->order_customer;
                                // $this->db->select('cname')->from('customers')->where('cid',$customer1);
                                // $cus = $this->db->get()->row();
                                // $customer_name = $cus->cname;
                                ...
                                {$status="<span class='label label-primary ' style='margin-bottom: 5px;margin-right: 5px' >Loaded </span>";}
                                    ...
                                    $status="<span class='label label-warning ' style='margin-bottom: 5px;margin-right: 5px' >Hold</span>";
                                }else{$status="<span class='label label-success ' style='margin-bottom: 5px;margin-right: 5px' >Deliverd</span>";}

                                
                                // if($r->order_customer == 0 && $r->order_customer ='null')
                                // {$customer="<span class='label label-primary ' style='margin-bottom: 5px;margin-right: 5px' >Customer Not Added</span>";}
                                // else
                                // {$customer="<span class='label label-primary ' style='margin-bottom: 5px;margin-right: 5px' >$customer_name</span>";}
                                ?>
                                <tr>
                                    ...
                                    <td><?php echo $r->delivery_date_time; ?></td>
                                    <td><?php echo $r->name; ?></td>
                                    <td> 
                                    <a id="btn_view" class="btn-icon-only btn btn-warning "  href="<?php echo base_url().$controller_directory?>/orders/order_details/<?php echo $r->order_id?>"><i class="fa fa-eye"></i></a>
                                   </a>
                                  <?php if($r->order_status == 4){ ?>
                                   <a id="confirmbtn" class="btn-icon-only btn btn-primary "  href="#confirm" data-toggle="modal"><i class="fa fa-exchange"></i></a>
                                   <?php } ?>
            ...
            <div class="modal-dialog ">
                <div class="modal-content">
                    <div class="alert alert-block alert-danger fade in">
                        ...
                        <p> Are You Sure to Confirm this Order ?. All the Associated Data will Sent To Godown. </p>
                        <form id="cinfirmform" action="" method="post" class="form-horizontal">
                        <p>
                            <button class="btn btn-success " type="submit" name="submit">Yes To Confirm</button>
                            <button type="button" class="btn btn-danger modal-dismiss" data-dismiss="modal">Cancel</button>
                        </p>
...
<script>
    ...
    $('body').on('click', '#deletebtn', function() {
        ...
        $("#deleteform").attr('action', '<?php echo base_url().$controller_directory ?>/orders/order_delete');
    });
    ...
    $('body').on('click', '#confirmbtn', function() {
        var no = $(this).closest('tr').children('td');
        $("#orderid").val(no.eq(0).text());
        $("#cinfirmform").attr('action', '<?php echo base_url().$controller_directory ?>/orders/order_confirm_billing');
    });
</script>