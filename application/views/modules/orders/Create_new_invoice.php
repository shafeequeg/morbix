...
<script>
    ...
    $('.btn-save-order').click(function() {
        if(orders.length) {
            ...
            const _xhr = $.post('<?= base_url() . $controller_directory; ?>/orders/invoice', {orders: orders, customer: customer});
            _xhr.done(function() {
                location.reload();
                toastr["success"]("Success . New Invoice Created", "Success")
            })
        }
        else{
